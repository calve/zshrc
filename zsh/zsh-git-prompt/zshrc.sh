...
git_super_status() {
#Just print the current branch (or where HEAD is) using colors
#blue if clean
#green if staged/changed
#red if untracked/unstaged/conflict
	...
	STATUS="$ZSH_THEME_GIT_PROMPT_PREFIX"
	  if [ -n "$GIT_REMOTE" ]; then
		  STATUS="$STATUS$ZSH_THEME_GIT_PROMPT_REMOTE"
	  fi
	  ...
	  STATUS="$STATUS$GIT_BRANCH$ZSH_THEME_GIT_PROMPT_SUFFIX"
	  echo "$STATUS"
	fi
}
