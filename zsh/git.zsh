...
#Need zsh-git-prompt
	...
	if [[ -n $(git rev-parse --show-prefix 2>/dev/null) ]]; then #Si on est dans un sous répertoire
	    THISDIR+="$(arrow black $ZSH_THEME_PWD_BG2)"
	    THISDIR+="%{$BG[$ZSH_THEME_PWD_BG2]%}%{$fg[black]%}$(git rev-parse --show-prefix)"
	    THISDIR+="$(arrow $ZSH_THEME_PWD_BG2 default)"
	else
	    THISDIR+="$(arrow black default)" 
	fi
    else
	PARENTDIR="%{$BG[$ZSH_THEME_PWD_BG1]%}%{$FG[0]%}$(dirname $(print -P %3~))"
	THISDIR="%{$BG[$ZSH_THEME_PWD_BG2]%}%{$fg[black]%}$(print -P %1~)$(arrow $ZSH_THEME_PWD_BG2 default)"
    fi
